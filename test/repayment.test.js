...
contract('Repayment', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        Repay = await Repayment.new(creditor,creditee,4,2,12,contract_addr)
    ...
    */
    ...
    it("Receiving a payment on time", async () =>{
       // const advancement = 604800; //7days
        ...
        const pay = 0.3*(10**18)
        //console.log("Due date "+parseInt(await Repay.dueDate()))
        //console.log("Current time  "+parseInt(await Repay.getNow()))
        const newBlock = await time.advanceTimeAndBlock(advancement);
        //console.log("time after going forward "+parseInt(await Repay.getNow()))
